...
#define EXTERNAL_FLASH_BASE OCTOSPI1_BASE
...
bool update_extflash(const char *path, extflash_progress_callback_t progress_callback)
{
    FIL file;
    UINT bytes_read;
    ...
    do
    {
        OSPI_EraseSync(flash_address, block_size);
        ...
        OSPI_Program(flash_address, buffer, bytes_read);

        OSPI_EnableMemoryMappedMode();
        // Verify written data
        if (memcmp((void *)(EXTERNAL_FLASH_BASE + flash_address), buffer, bytes_read) != 0)
        {
            printf("Verification failed at address 0x%08lx\n", flash_address);
            update_status = false;
            break;
        }
        OSPI_DisableMemoryMappedMode();
        ...
        if (progress_callback) {
            unsigned int percentage = (unsigned int)(((flash_address) * 100) / f_size(&file));
